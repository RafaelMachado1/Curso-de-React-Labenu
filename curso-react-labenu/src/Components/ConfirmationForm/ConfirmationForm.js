...
const ConfirmationForm = ({setFormFlow}) => {
  ...
  //função que vetifica se todos os campos foram preenchidos,  envia os dados e apaga os inputs ou caso
  //algum campo não esteja preenchido emite um alert.
  ...
  return (
    <Form>
      ...
      {/* Outra forma de trabalhar com o select é eniando valores diferentes das opções disponíveis 
      usando em cada tag option um atributo value, neste caso o estado 'cor' irá receber oa valores do 
      atributo value. */}
      <label>
        {" "}
        Cor:
    ...
    */}
      <section>
        ...
        {/* Se o intuito for escolher entre uma OU outra opção, como  um curso noturno ou Integral, 
        mas nunca os dois, passamos o mesmo valor para o atributo 'name'. o atributo checkd existe 
        aqui para iniciar uma das opções marcadas logo de inicio. neste caso a apção Noturno inicia marcado.  */}
        <input
          type="radio"
          id="curso1"
          name="curso"
      ...
      <section>
        {/* Já no input checkbox, para que o botão inicie desativado o atributo checked deve receber como 
        valor false. aqui passamos o estado que iniciará com este valor.
        Se o intuito for multipla escolha como abaixo, podemos passar  valores diferentes para o atributo 
        'name'   */}
        <input
          type="checkbox"
          id="html"
